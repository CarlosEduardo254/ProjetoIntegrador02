...
public static class CoordenadorRotas
{
    public static void CoordenadorRota(this WebApplication app)
    {
        ...
        // Obter coordenador de um usuário líder
        rota.MapGet("obterCoordenador/{idLider}", async (Guid idLider, DbContextApp context) =>
        {
            var coordenador = await context.Coordenadores
                .FirstOrDefaultAsync(c => c.UsuarioId == idLider);
        
            if (coordenador == null) return Results.NotFound("Coordenador não encontrado");
    
            var coordenadorDto = new CoordenadorRespostaDto
            (
                coordenador.Nome,
                coordenador.Email,
                coordenador.Contato
            ...
            return Results.Ok(coordenadorDto);
        });
